...
object Db {
    ...
    fun deleteDatabase(userId: Long? = null) {
        close()
        val dbName = if (userId != null) "fortrx_$userId.db" else Settings.dbFilePath
        println("Db: Deleting database file $dbName")
    ...
    suspend fun saveKeys(password: String, userId: Long, keysJson: String) = withContext(Dispatchers.Default) {
        println("Db: Saving encrypted keys for user $userId to private_keys table")
    ...
    suspend fun loadKeys(password: String, userId: Long?): String? = withContext(Dispatchers.Default) {
        println("Db: Querying private_keys table for user $userId")
        ...
        if (blob == null) {
            println("Db: No record found in private_keys table")
            return@withContext null
        }
        
        try {
            val decrypted = decrypt(blob, password).decodeToString()
            println("Db: Successfully decrypted keys")
            decrypted
        } catch (e: Exception) {
            println("Db: FAILED to decrypt keys: ${e.message}")
            null
        }
    }
    ...
    suspend fun saveIncomingMessage(password: String, serverMessageId: Long?, contactId: kotlin.Long, senderId: Long?,
        ...
        try {
            database.transaction {
                ...
                q.upsertConversationSummary(contactId, null, createdAt, null, preview, "incoming", status, unread)
            }
        } catch (t: Throwable) {
        ...
        try {
            database.transaction {
                if (serverMessageId != null && q.existsServerMessage(serverMessageId).executeAsOne()) {
                    ...
                    q.upsertConversationSummary(contactId, null, createdAt, null, preview, "outgoing", status, 0)
                }
            }
        } catch (t: Throwable) {
